...
class Census:
    ...
    def _read_data(self, path=None, data_file=None):
        ...
        self.data = pd.read_csv(
            os.path.join(data_path, clean_data_file), nrows=self.nrows
        )
    ...
    def _process_data(
        self, features=None, training_flag: bool = True, encoder=None, lb=None
    ):
        """
        ...
        features, target, encoder, lb = process_data(
            input_features,
    ...
    def execute_training(self):
        ...
        self._read_data()
        self._split_data()
        self.X_train, self.y_train, self.encoder, self.lb = self._process_data(
            training_flag=True
        )

        self.X_test, self.y_test, _, _ = self._process_data(
            training_flag=False, features=self.test, encoder=self.encoder, lb=self.lb
        )
    ...
    def execute_inference(self, model, encoder, lb, df: pd.DataFrame = None):
        ...
        features, _, _, _ = self._process_data(
            features=df, training_flag=False, encoder=encoder, lb=lb
        )
